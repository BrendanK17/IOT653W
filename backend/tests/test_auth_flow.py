...
def test_auth_full_flow():
    ...
    with patch(
        "app.auth.services.find_user_by_email", return_value=None
    ) as mock_find_user, patch(
        "app.auth.services.create_user"
    ) as mock_create_user, patch(
        "app.auth.services.generate_and_store_verification_token",
        return_value="fake_token",
    ), patch(
        "app.auth.services.confirm_verification_token", return_value=True
    ), patch(
        "app.auth.services.verify_password", return_value=True
    ), patch(
        "app.auth.services.create_access_and_refresh_tokens",
        return_value={"access_token": "access", "refresh_token": "refresh"},
    ), patch(
        "app.auth.services.generate_and_store_password_reset_token",
        return_value="reset_token",
    ), patch(
        "app.auth.services.confirm_and_consume_reset_token", return_value=True
    ), patch(
        "app.auth.services.revoke_all_user_refresh_tokens"
    ):
